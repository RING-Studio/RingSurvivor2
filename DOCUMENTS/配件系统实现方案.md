...
# 配件
{
    "id": "mine",
    "name": "地雷",
    "description": "每{冷却时间}秒放置一个地雷，接触敌人后对{爆炸范围}范围内敌人造成{基础伤害=5lv}点伤害。部署上限：10+5lv",
    "quality": "blue",
    "max_level": 4,
...
{
    "id": "accessory_cooldown",
    "name": "配件冷却",
    "description": "所有配件冷却时间缩短{accessory_cooldown_value}%",
    "quality": "red",
    "max_level": 3,
    "upgrade_type": "accessory",
    "exclusive_for": ""
...
},
{
    "id": "mine_anti_armor",
    "name": "地雷·反甲",
    "description": "地雷冷却时间+5秒，地雷基础伤害+{mine_anti_armor_value}%。地雷仅能由精英或BOSS触发",
    "quality": "purple",
    "max_level": 5,
    "upgrade_type": "enhancement",
    "exclusive_for": "mine"
...
```gdscript
func _ready():
    # 1. 初始化升级数据库（包含所有升级，包括专属强化）
    _initialize_upgrade_database()
    
    # 2. 初始化局内升级池（只加入非专属升级和已满足条件的专属升级）
    _initialize_active_pools()
    
    GameEvents.level_up.connect(on_level_up)

func _initialize_upgrade_database():
    """初始化升级数据库（所有升级条目）"""
...
func _initialize_active_pools():
    ...
    for upgrade_id in upgrade_database:
        ...
        if exclusive_for == "mg":
            ...
            should_add = ("mine" in session_equipped_accessories)
...
@export var base_explosion_radius: float = 50.0  # 基础爆炸范围（像素）
...
func _get_cooldown() -> float:
    ...
    # 地雷·反甲：+5秒
    var anti_armor_level = GameManager.current_upgrades.get("mine_anti_armor", {}).get("level", 0)
    if anti_armor_level > 0:
        cooldown += 5.0
    
    # 全局配件冷却：-10% per level
    var accessory_cooldown_level = GameManager.current_upgrades.get("accessory_cooldown", {}).get("level", 0)
    if accessory_cooldown_level > 0:
        cooldown_reduction += UpgradeEffectManager.get_effect("accessory_cooldown", accessory_cooldown_level)
...
func _get_explosion_radius() -> float:
    ...
    if range_level > 0:
        radius += UpgradeEffectManager.get_effect("mine_range", range_level) * 32.0  # 转换为像素（假设1米=32像素）
    return radius
...
func _get_base_damage() -> float:
    ...
    var anti_armor_level = GameManager.current_upgrades.get("mine_anti_armor", {}).get("level", 0)
    if anti_armor_level > 0:
        var multiplier = UpgradeEffectManager.get_effect("mine_anti_armor", anti_armor_level)
...
func _deploy_mine(position: Vector2):
    ...
    mine_instance.setup(
        ...
        _get_base_damage(),
        GameManager.current_upgrades.get("mine_anti_armor", {}).get("level", 0) > 0
    )
...
func _on_upgrade_added(upgrade_id: String, current_upgrades: Dictionary):
    """升级添加时更新"""
    if upgrade_id in ["mine", "mine_cooldown", "mine_anti_armor", "accessory_cooldown"]:
        mine_timer.wait_time = _get_cooldown()
        mine_timer.start()
```
...
### 4.1 DamageTextHelper 扩展

```gdscript
var COLORS := {
    "weapon": Color(1, 1, 1, 1),  # 白色
    "critical": Color(1, 0.93, 0.2, 1),  # 黄色
    "accessory": Color(1, 1, 1, 1),  # 白色（与武器伤害相同）
    "accessory_critical": Color(1, 0.93, 0.2, 1),  # 黄色（与武器暴击相同）
