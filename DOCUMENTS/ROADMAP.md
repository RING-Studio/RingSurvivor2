...
## 阶段 15：升级扩充第三批 ✅ 已完成（2026-02-19）
目标
- 将文档中有设计+有图标但无代码的 17 个升级落地
主要工作
- 15.1 AbilityUpgradeData.gd 新增 17 个 entry（升级合集遗留 8 + 扩充清单v1 B区 9）— ✅
- 15.2 UpgradeEffectManager.gd 新增 17 个效果 config — ✅
- 15.3 upgrade_icons 字典接入 17 个已有 PNG — ✅
...
| 问题 | 状态 | 说明 |
|------|------|------|
| 孤立场景：`Contamination/` 和 `SalvageRun/` | ✅ 已清理 | 两个早期原型场景未被任何任务路由，`ContaminationLevel.tscn` 引用不存在的 `SalvageRunLevel.gd`。已删除两个目录 |
...
| 问题 | 状态 | 说明 |
|------|------|------|
| `player.gd` 有 7+ 处 debug print | ✅ 已清理 | 移除了所有非必要的 `print()` 调用 |
| `player.gd` 死代码分支 | ✅ 已清理 | 移除 `global_health_1/2/3/4`、`global_crit_rate_1/2` 等旧分支 |
| `player.gd` 注释掉的动画代码 | ✅ 已清理 | 移除行 269-274 的注释代码块 |

### 待实现升级清单（文档有设计但代码未落地） → ✅ 阶段15已落地

17 个升级在阶段 15 中完成了数据 entry、效果 config、图标映射。

**已完成gameplay效果（stat类）：**
- `thermal_imager`（暴击率+5%）、`laser_rangefinder`（暴击率+4%）— player.gd 全局暴击
- `extra_ammo_rack`（耐久/移速减益，射速/装填加成）— player.gd + WeaponUpgradeHandler
- `long_barrel`（穿透+，射速+，伤害-）、`tandem_heat`（穿透+，对护甲伤害+）— WeaponUpgradeHandler
...
- NPC 对话进度推进（跨存档持久化）
- 配件解锁流程（选取→解锁→CarEditor显示→结算新解锁）
- 结算带出物品系统（能量返还、素材损失比例）
- 三张关卡场景稳定性（LevelTest/LevelDesert/LevelContaminated 各跑10分钟）
...
- 修复 4 处 `:=` 违规：`player.gd`、`SaveData.gd`(2x)、`upgrade_screen.gd`
- 清理 `player.gd` 中 7+ 处 debug print、死代码分支（`global_health_*`/`global_crit_rate_*`）、注释动画代码
