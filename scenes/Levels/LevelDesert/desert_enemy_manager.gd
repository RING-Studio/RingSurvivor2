extends Node
## 沙漠关卡敌人管理器 — 大型场地、中等密度、无 Boss

const SPAWN_RADIUS: int = 1200  # 大型关卡（19200×10800）生成半径
...
func _ready() -> void:
	...
	# 沙漠关卡的敌人权重：偏向基础近战
	enemy_table.add_item(sand_scarab_scene, 5)
...
func _on_arena_difficulty_increased(arena_difficulty: int) -> void:
	...
	# 精英波事件：每隔一段时间触发一波精英
	if (arena_difficulty % 10) == 0 and arena_difficulty > 0:
		_spawn_elite_wave(2 + arena_difficulty / 10)
...
func calculate_max_random_enemies() -> int:
	...
	# 大型关卡（19200×10800）：更高的生成上限，确保大地图不空旷
	return int(18 + point_pollution / 600.0) + bonus
