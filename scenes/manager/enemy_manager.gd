...
func on_timer_timeout():
	...
	for i in number_to_spawn:
		...
		var difficulty = arena_time_manager.arena_difficulty
		initialize_enemy_properties(enemy, difficulty)
...
func initialize_enemy_properties(enemy: Node2D, difficulty: int):
	"""初始化敌人属性，使用 difficulty 参数"""
	var properties = {}
	var multiplier = 1.0 + (difficulty * 0.1)  # 基础倍率随难度增加
