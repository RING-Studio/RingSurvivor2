extends Node
class_name ClusterMineAbilityController
...
@export var sub_radius_m: float = 1.2
@export var trigger_delay_seconds: float = 0.8
...
func _on_upgrade_added(upgrade_id: String, _current: Dictionary):
	if upgrade_id in ["cluster_mine", "cooling_device", "cluster_count", "cluster_radius", "damage_bonus", "crit_damage", "crit_rate"]:
		_update_timer()

func _get_cooldown_seconds() -> float:
	var lvl = GameManager.current_upgrades.get("cluster_mine", {}).get("level", 0)
	if lvl <= 0:
		return INF
...
func _get_base_damage() -> float:
	var lvl = GameManager.current_upgrades.get("cluster_mine", {}).get("level", 0)
	return 6.0 + 3.0 * float(max(lvl, 1))

func _get_base_radius_px() -> float:
	return base_radius_m * GlobalFomulaManager.METERS_TO_PIXELS

func _get_sub_count() -> int:
	var count_level = GameManager.current_upgrades.get("cluster_count", {}).get("level", 0)
	return 3 + int(UpgradeEffectManager.get_effect("cluster_count", count_level))

func _get_sub_radius_px() -> float:
	var bonus_level = GameManager.current_upgrades.get("cluster_radius", {}).get("level", 0)
	var radius_m = sub_radius_m + UpgradeEffectManager.get_effect("cluster_radius", bonus_level)
...
func _on_cooldown():
	...
	var center = player.global_position + offset
	var radius_px = _get_base_radius_px()
	var base_damage = _get_base_damage()
	var sub_count = _get_sub_count()
	var sub_radius_px = _get_sub_radius_px()
	
	var layer = get_tree().get_first_node_in_group("foreground_layer")
	if layer:
		var warn = AoECircleEffect.new()
		warn.global_position = center
		warn.setup(radius_px, Color(1.0, 0.6, 0.2, 40.0 / 255.0), trigger_delay_seconds)
		layer.add_child(warn)
	
	var t = get_tree().create_timer(trigger_delay_seconds)
	t.timeout.connect(func():
		_explode_cluster(center, radius_px, base_damage, sub_count, sub_radius_px)
	)

func _explode_cluster(center: Vector2, radius_px: float, base_damage: float, sub_count: int, sub_radius_px: float) -> void:
	var layer = get_tree().get_first_node_in_group("foreground_layer")
	if layer:
	...
	await _apply_area_damage(center, radius_px, base_damage)
	
	for i in range(sub_count):
		var offset = Vector2.RIGHT.rotated(randf_range(0.0, TAU)) * randf_range(0.0, radius_px * 0.6)
		var sub_center = center + offset
		if layer:
			var sub_fx = AoECircleEffect.new()
			sub_fx.global_position = sub_center
			sub_fx.setup(sub_radius_px, Color(1.0, 0.4, 0.2, 64.0 / 255.0), 0.18)
			layer.add_child(sub_fx)
		await _apply_area_damage(sub_center, sub_radius_px, base_damage * 0.6)

func _apply_area_damage(center: Vector2, radius_px: float, base_damage: float) -> void:
	var layer = get_tree().get_first_node_in_group("foreground_layer")
