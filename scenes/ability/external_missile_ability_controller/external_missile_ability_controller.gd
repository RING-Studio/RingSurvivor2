...
func _on_upgrade_added(upgrade_id: String, _current: Dictionary):
	if upgrade_id in ["external_missile", "missile_damage", "cooling_device", "damage_bonus", "crit_rate", "crit_damage"]:
		_update_timer()
...
func _on_timeout():
	...
	var lock_px := lock_range_m * GlobalFomulaManager.METERS_TO_PIXELS
	...
	if missile.has_method("setup"):
		missile.setup(target, _get_base_damage(), explosion_radius_m * GlobalFomulaManager.METERS_TO_PIXELS)
