extends Node
class_name JammerFieldAbilityController
...
func _on_upgrade_added(upgrade_id: String, _current: Dictionary):
	if upgrade_id in ["jammer_field", "cooling_device", "jammer_radius", "jammer_intensity"]:
		_update_timer()

func _get_cooldown_seconds() -> float:
	var lvl = GameManager.current_upgrades.get("jammer_field", {}).get("level", 0)
	if lvl <= 0:
		return INF
...
func _get_radius_pixels() -> float:
	var bonus_lvl = GameManager.current_upgrades.get("jammer_radius", {}).get("level", 0)
	var radius_m = base_radius_m + UpgradeEffectManager.get_effect("jammer_radius", bonus_lvl)
	return radius_m * GlobalFomulaManager.METERS_TO_PIXELS

func _get_duration_seconds() -> float:
	var lvl = GameManager.current_upgrades.get("jammer_field", {}).get("level", 0)
	var duration = 3.0 + 0.5 * float(max(lvl, 1))
	var bonus_lvl = GameManager.current_upgrades.get("jammer_intensity", {}).get("level", 0)
	duration += UpgradeEffectManager.get_effect("jammer_intensity", bonus_lvl)
	return duration
...
func _on_cooldown():
	...
	if layer:
		...
		fx.setup(radius_px, Color(0.6, 0.2, 0.9, 40.0 / 255.0), 0.3)
		layer.add_child(fx)
